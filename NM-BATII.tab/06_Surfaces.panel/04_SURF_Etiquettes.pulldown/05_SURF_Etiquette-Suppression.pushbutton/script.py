...
CONFIG_KEY = 'suppression_masse_etiquettes_surfaces'
...
def _is_placed_area(area):
    """Une surface non placée n'a pas de point d'insertion. Le test porte sur
    LocationPoint, et pas seulement sur la présence d'une Location, comme dans
    lib/utils/surfaces_etiquettes.py."""
    try:
...
def _tag_area(tag):
    ...
    (orpheline, liée...). Renoncer trop tôt reviendrait ici à épargner une
    étiquette que l'utilisateur a pourtant désignée."""
...
# ─── Collecte ─────────────────────────────────────────────────────────────────
def get_used_area_tag_type_ids():
    """IntegerValue des types d'étiquette réellement utilisés par au moins une
    étiquette de surface du projet."""
    ids = set()
    for tag in get_project_area_tags():
        try:
            tag_type = tag.AreaTagType
            if tag_type is not None:
                ids.add(tag_type.Id.IntegerValue)
        except Exception:
            pass
    return ids
...
def get_area_tag_types():
    """Retourne les FamilySymbol de la catégorie 'Étiquette de surface'
    réellement utilisés par au moins une étiquette du projet, triés."""
    used_ids = get_used_area_tag_type_ids()
    types = list(
    ...
    types = [t for t in types if t.Id.IntegerValue in used_ids]
    types.sort(key=lambda t: (_elem_name(t.Family), _elem_name(t)))
    return types


def get_project_area_tags():
    return list(
        DB.FilteredElementCollector(doc)
        .OfCategory(DB.BuiltInCategory.OST_AreaTags)
...
def filter_tags_by_areas(tags, area_ids):
    result = []
    for tag in tags:
        try:
            area = _tag_area(tag)
            if area is not None and area.Id.IntegerValue in area_ids:
                result.append(tag)
        except Exception:
            pass
    return result
...
def filter_tags_by_types(tags, type_ids):
    result = []
    for tag in tags:
        try:
            tag_type = tag.AreaTagType
            if tag_type is not None and tag_type.Id.IntegerValue in type_ids:
                result.append(tag)
        except Exception:
            pass
    return result
...
# Les surfaces sont des SpatialElement : leurs paramètres « Numéro », « Nom » et
# « Niveau » sont les MÊMES built-in que ceux des pièces (ROOM_*), malgré leur
# nom. AREA_SCHEME_ID, lui, n'existe que pour elles — et il compte : deux
# surfaces homonymes de schémas différents ne se distinguent que par là.
_BIP_AREA_NUMBER = _safe_bip('ROOM_NUMBER')
_BIP_AREA_NAME   = _safe_bip('ROOM_NAME')
_BIP_AREA_LEVEL  = _safe_bip('ROOM_LEVEL_ID')
_BIP_AREA_SCHEME = _safe_bip('AREA_SCHEME_ID')
...
def _builtin_param_value_str(elem, bip):
    if bip is None:
        return u''
    try:
...
def _elem_family_and_type(elem):
    """Famille/Type de l'élément lui-même (ex. vue : 'Plan d'étage' /
    '1:50 Bâti'), lus sur son ElementType plutôt que via LookupParameter :
    indépendant de la langue de Revit et toujours disponible (pas un
    paramètre standard). Renvoie ('', '') si l'élément n'a pas de type
    exploitable (ex. les surfaces, qui n'ont pas de FamilySymbol)."""
    try:
    ...
    if et is None:
        return u'', u''
    try:
        family = et.FamilyName or u''
    except Exception:
        family = u''
    try:
        type_name = Element.Name.__get__(et) or u''
    except Exception:
        type_name = u''
...
def _area_fixed_columns_spec():
    """Libellé (localisé) + clé de binding des colonnes fixes du tableau de
    surfaces, dans l'ordre Numéro / Nom / Niveau / Schéma / Famille / Type."""
    return [
        (_bip_label(_BIP_AREA_NUMBER, u'Numéro'), u'Number'),
        (_bip_label(_BIP_AREA_NAME, u'Nom'), u'Name'),
        (_bip_label(_BIP_AREA_LEVEL, u'Niveau'), u'LevelName'),
        (_bip_label(_BIP_AREA_SCHEME, u'Schéma de surface'), u'SchemeName'),
        (_bip_label(_BIP_FAMILY, u'Famille'), u'Family'),
        (_bip_label(_BIP_TYPE, u'Type'), u'TypeName'),
    ]


def _area_fixed_column_labels():
    return set(label for label, key in _area_fixed_columns_spec())
...
class _ViewRow(object, INotifyPropertyChanged):
    ...
    def __init__(self, view):
        ...
        self.Family, self.TypeName = _elem_family_and_type(view)
...
def show_views_picker_dialog(all_views, current_selected_ids):
    ...
    # "Nom de la vue"/"Niveau associé"/"Echelle de la vue"/"Famille"/"Type"
    # sont déjà couvertes par les colonnes fixes ViewName/AssociatedLevel/
    # ViewScale/Family/TypeName (résolues par BuiltInParameter) : on les
    # retire des colonnes dynamiques pour éviter un doublon.
...
# ─── Surfaces sélectionnées : collecte, tableau de choix ───────────────────────
def get_all_project_areas():
    """Toutes les surfaces placées (avec point d'insertion) du projet."""
    areas = list(
        DB.FilteredElementCollector(doc)
...
def _area_sort_key(row):
    """Clé de tri par défaut du tableau de surfaces : niveau puis numéro."""
    try:
        level = row.area.Level
        level_name = _elem_name(level) if level else u''
    except Exception:
        level_name = u''
    try:
        number = row.area.Number or u''
    except Exception:
        number = u''
    return (_normalize(level_name), _normalize(number))


class _AreaRow(object, INotifyPropertyChanged):
    """Ligne du tableau de choix des surfaces (voir _ViewRow)."""

    def __init__(self, area):
        self.area = area
        self._selected = False
        self._PropertyChanged = None
        self.Number = _builtin_param_value_str(area, _BIP_AREA_NUMBER)
        self.Name = _builtin_param_value_str(area, _BIP_AREA_NAME)
        self.LevelName = _builtin_param_value_str(area, _BIP_AREA_LEVEL)
        self.SchemeName = _builtin_param_value_str(area, _BIP_AREA_SCHEME)
        self.Family, self.TypeName = _elem_family_and_type(area)
...
def build_area_rows(areas, ordered_names):
    rows = []
    for a in areas:
        row = _AreaRow(a)
        for i, name in enumerate(ordered_names):
            setattr(row, 'P{}'.format(i), _param_value_str(a, name))
        rows.append(row)
    return rows


def show_areas_picker_dialog(all_areas, current_selected_ids):
    """Tableur de choix des surfaces à traiter : mêmes options que
    show_views_picker_dialog (filtres/tri par colonne, sélection multiple
    Maj+Clic), colonnes Numéro/Nom/Niveau/Schéma en tête, triées par défaut
    par niveau puis numéro."""
    xaml_path = os.path.join(os.path.dirname(__file__), 'AreasPickerDialog.xaml')
    wpf = forms.WPFWindow(xaml_path)

    # "Numéro"/"Nom"/"Niveau"/"Schéma de surface"/"Famille"/"Type" sont déjà
    # couvertes par les colonnes fixes Number/Name/LevelName/SchemeName/Family/
    # TypeName (résolues par BuiltInParameter) : on les retire des colonnes
    # dynamiques pour éviter un doublon.
    raw_names = [n for n in get_dynamic_param_names(all_areas) if n not in _area_fixed_column_labels()]
    ordered_names = order_column_names(raw_names, pinned_order=[])
    all_rows = build_area_rows(all_areas, ordered_names)
    all_rows.sort(key=_area_sort_key)
    for r in all_rows:
        r.Selected = r.area.Id.IntegerValue in current_selected_ids
    ...
    def _update_count():
        n = sum(1 for r in all_rows if r.Selected)
        wpf.txtCount.Text = u"({} surface(s) sélectionnée(s))".format(n)
    ...
    for label, key in _area_fixed_columns_spec():
        col = DataGridTextColumn()
        col.Header = _make_header(label, key)
        col.Binding = Binding(key)
        col.IsReadOnly = True
    ...
    if not dialog_result['ok']:
        return None
    return set(r.area.Id.IntegerValue for r in all_rows if r.Selected)
...
def main():
    ...
    if not tag_types:
        show_alert(
            u"Supprimer les étiquettes",
            u"Aucune étiquette de surface n'est utilisée dans ce projet."
        )
        return
    ...
    selection_state_projet = {'areas': []}
    selection_state_vues = {'areas': []}
    views_state = {'selected_view_ids': set(), 'candidate_views': None}
    areas_state = {'candidate_areas': None}
    dialog_result = {'ok': False}
    ...
    def _mk_select_areas_handler(state, txt_name, target_radio):
        def handler(s, e):
            ...
            try:
                ...
                state['areas'] = [a for a in areas if a is not None and _is_placed_area(a)]
            except Exception:
                pass
            wpf.WindowState = WindowState.Normal
            wpf.Activate()
            getattr(wpf, txt_name).Text = u"({} surface(s) sélectionnée(s))".format(len(state['areas']))
            target_radio.IsChecked = True
        return handler

    def _mk_select_areas_table_handler(state, txt_name, target_radio):
        def handler(s, e):
            """Sélection des surfaces via un tableau (comme 'Choisir les vues...'),
            plutôt qu'en désignant les surfaces dans une vue Revit."""
            if areas_state['candidate_areas'] is None:
                areas_state['candidate_areas'] = get_all_project_areas()
            candidates = areas_state['candidate_areas']
            if not candidates:
                show_alert(u"Supprimer les étiquettes",
                           u"Aucune surface placée n'est disponible dans ce projet.")
                return
            current_ids = set(a.Id.IntegerValue for a in state['areas'])
            result = show_areas_picker_dialog(candidates, current_ids)
            wpf.Activate()
            if result is not None:
                state['areas'] = [a for a in candidates if a.Id.IntegerValue in result]
                getattr(wpf, txt_name).Text = u"({} surface(s) sélectionnée(s))".format(len(state['areas']))
                target_radio.IsChecked = True
        return handler
        ...
        def handler(s, e):
            ...
            if result is not None:
                ...
                if not (wpf.rbVuesSelectionneesToutes.IsChecked or
                        wpf.rbVuesSelectionneesAreas.IsChecked):
                    target_radio.IsChecked = True
        return handler

    def _on_ok(s, e):
        if not any(cb.IsChecked for cb, t, label in checkbox_items):
            show_alert(u"Supprimer les étiquettes", u"Sélectionnez au moins un type d'étiquette.")
            return
        if wpf.rbProjetSelection.IsChecked and not selection_state_projet['areas']:
            show_alert(u"Supprimer les étiquettes",
                       u"Aucune surface sélectionnée. Cliquez sur \"Sélectionner\".")
            return
        if wpf.rbVuesSelectionneesToutes.IsChecked and not views_state['selected_view_ids']:
            show_alert(u"Supprimer les étiquettes",
                       u"Aucune vue sélectionnée. Cliquez sur \"Choisir les vues...\".")
            return
        if wpf.rbVuesSelectionneesAreas.IsChecked:
            if not views_state['selected_view_ids']:
                show_alert(u"Supprimer les étiquettes",
                           u"Aucune vue sélectionnée. Cliquez sur \"Choisir les vues...\".")
                return
            if not selection_state_vues['areas']:
                show_alert(u"Supprimer les étiquettes",
                           u"Aucune surface sélectionnée. Cliquez sur \"Sélectionner\".")
                return
    ...
    def _update_scope_buttons(s=None, e=None):
        wpf.btnSelectAreasProjet.IsEnabled = bool(wpf.rbProjetSelection.IsChecked)
        wpf.btnSelectViews.IsEnabled = bool(wpf.rbVuesSelectionneesToutes.IsChecked)
        wpf.btnSelectViews2.IsEnabled = bool(wpf.rbVuesSelectionneesAreas.IsChecked)
        wpf.btnSelectAreasVues.IsEnabled = bool(wpf.rbVuesSelectionneesAreas.IsChecked)
        wpf.btnSelectAreas.IsEnabled = bool(wpf.rbVueSelection.IsChecked)

    for rb_name in (u'rbProjetToutes', u'rbProjetSelection',
                    u'rbVuesSelectionneesToutes', u'rbVuesSelectionneesAreas',
                    u'rbVueToutes', u'rbVueSelection'):
        getattr(wpf, rb_name).Checked += _update_scope_buttons
    _update_scope_buttons()

    wpf.btnDeselectAll.Click += _on_deselect_all
    wpf.btnSelectAreasProjet.Click += _mk_select_areas_table_handler(
        selection_state_projet, 'txtSelectionCountProjet', wpf.rbProjetSelection)
    wpf.btnSelectAreasVues.Click += _mk_select_areas_table_handler(
        selection_state_vues, 'txtSelectionCountVues', wpf.rbVuesSelectionneesAreas)
    wpf.btnSelectAreas.Click += _mk_select_areas_handler(
        selection_state, 'txtSelectionCount', wpf.rbVueSelection)
    wpf.btnSelectViews.Click += _mk_select_views_handler(wpf.rbVuesSelectionneesToutes)
    wpf.btnSelectViews2.Click += _mk_select_views_handler(wpf.rbVuesSelectionneesAreas)
    wpf.btnOK.Click += _on_ok
    wpf.btnCancel.Click += lambda s, e: wpf.Close()
    ...
    selected_type_ids = set(t.Id.IntegerValue for t, label in selected)
    _save_default_tag_types([label for t, label in selected])

    if wpf.rbProjetToutes.IsChecked:
        scope = 'projet_toutes'
    elif wpf.rbProjetSelection.IsChecked:
        scope = 'projet_selection'
    elif wpf.rbVuesSelectionneesToutes.IsChecked:
        scope = 'vues_selection_toutes'
    elif wpf.rbVuesSelectionneesAreas.IsChecked:
        scope = 'vues_selection_areas'
    elif wpf.rbVueToutes.IsChecked:
        scope = 'vue_toutes'
    else:
        scope = 'vue_selection'

    # Liste de couples (vue, étiquettes à supprimer dans cette vue).
    view_tags_pairs = []

    if scope == 'projet_toutes':
        tags = filter_tags_by_types(get_project_area_tags(), selected_type_ids)
        if tags:
            view_tags_pairs.append((None, tags))
    elif scope == 'projet_selection':
        area_ids = set(r.Id.IntegerValue for r in selection_state_projet['areas'])
        tags = filter_tags_by_types(
            filter_tags_by_areas(get_project_area_tags(), area_ids), selected_type_ids
        )
        if tags:
            view_tags_pairs.append((None, tags))
    elif scope == 'vues_selection_toutes':
        ...
        for v in candidates:
            if v.Id.IntegerValue not in selected_ids:
                continue
            tags = filter_tags_by_types(get_view_area_tags(v), selected_type_ids)
            if tags:
                view_tags_pairs.append((v, tags))
    elif scope == 'vues_selection_areas':
        selected_ids = views_state['selected_view_ids']
        candidates = views_state['candidate_views'] or []
        area_ids = set(r.Id.IntegerValue for r in selection_state_vues['areas'])
        for v in candidates:
            if v.Id.IntegerValue not in selected_ids:
                continue
            tags = filter_tags_by_types(
                filter_tags_by_areas(get_view_area_tags(v), area_ids), selected_type_ids
            )
            if tags:
                view_tags_pairs.append((v, tags))
    elif scope == 'vue_toutes':
        tags = filter_tags_by_types(get_view_area_tags(view), selected_type_ids)
        if tags:
            view_tags_pairs.append((view, tags))
    else:
        area_ids = set(r.Id.IntegerValue for r in selection_state['areas'])
        tags = filter_tags_by_types(
            filter_tags_by_areas(get_view_area_tags(view), area_ids), selected_type_ids
        )
        if tags:
            view_tags_pairs.append((view, tags))

    if not view_tags_pairs:
        show_alert(u"Supprimer les étiquettes", u"Aucune étiquette à supprimer selon les critères choisis.")
        return

    n_ok = 0
    n_skip = 0
    with revit.Transaction(u"Supprimer les étiquettes de surfaces en masse"):
        for _target_view, tags in view_tags_pairs:
            for tag in tags:
                try:
                    doc.Delete(tag.Id)
                    n_ok += 1
    ...
    msg = u"{} étiquette(s) supprimée(s).".format(n_ok)
    if n_skip:
        msg += u"\n{} étiquette(s) ignorée(s) (échec de suppression).".format(n_skip)
    show_alert(u"Supprimer les étiquettes", msg)
...
class _ActionHandler(IExternalEventHandler):
    ...
    def GetName(self):
        return u"NM-BATII — Suppression des etiquettes de surfaces"
