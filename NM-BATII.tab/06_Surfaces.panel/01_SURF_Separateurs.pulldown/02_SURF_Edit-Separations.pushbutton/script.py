...
#__title__ = 'Edition séparateurs de surfaces'
#__author__ = 'data8bim (d8b)'


"""
NM-BATII — Mode edition des separateurs de surfaces.

But : corriger rapidement les separateurs de surface sans risquer de deplacer
ou supprimer un mur, un DWG ou quoi que ce soit d'autre.

Les surfaces (Area) ne vivent que dans les PLANS DE SURFACE : le mode ne s'arme
donc que la, contrairement a son jumeau des pieces qui accepte tout plan.
...
class _ActionHandler(IExternalEventHandler):
    ...
    def GetName(self):
        return u"NM-BATII — Mode édition des séparateurs de surfaces"
...
class FenetreMode(forms.WPFWindow):
    ...
    def __init__(self):
        ...
        # Etat de la case « Verrouiller aussi les surfaces », CAPTURE au moment
        # d'armer le mode et non relu pendant l'epinglage : c'est le reglage en
        # vigueur a l'activation qui doit valoir, et il sert aussi a la
        # desactivation, alors que la case est deja regrisee.
        self._verrouiller_surfaces = True
    ...
    def _chemin_recuperation(self, doc):
        ...
        try:
            ...
            return os.path.join(tempfile.gettempdir(),
                                u"NM-BATII_mode-limites-surfaces_{}.json".format(nom))
        except Exception:
            return None
    ...
    def _verifier_recuperation(self):
        ...
        try:
            ...
            reponse = show_confirm(
                u"NM-BATII — Édition des séparateurs de surfaces",
    ...
    def _rafraichir_etat(self):
        ...
        try:
            ...
            self.chk_surfaces.IsEnabled = not self._actif
            self._afficher()
        except Exception:
            pass
    ...
    _AIDE = (
        u"Ce mode protège tout, sauf les séparateurs de surfaces.\n\n"
        u"À l'activation :\n"
        ...
        u"« Verrouiller aussi les surfaces » commande le sort des surfaces "
        u"elles-mêmes : cochée, elles sont épinglées comme le reste ; décochée, "
        u"elles restent sélectionnables, pour ajuster un contour et sa surface "
        u"dans la même passe. L'option se règle avant d'activer le mode.\n\n"
        u"Le bouton à l'icône lance l'outil natif Revit « Limite de "
        u"surface » pour tracer de nouveaux séparateurs."
    )
    ...
    def basculer(self):
        ...
        if self._actif:
            ...
            try:
                self._verrouiller_surfaces = bool(self.chk_surfaces.IsChecked)
            except Exception:
                self._verrouiller_surfaces = True
    ...
    def btn_separateur_Click(self, sender, args):
        """
        Lance l'outil natif Revit « Limite de surface ».

        AreaBoundary est un membre de l'enumeration PostableCommand : il est
    ...
    def _activer(self):
        """Epingle tout sauf les separateurs de surfaces, puis coupe la selection."""
        try:
        ...
        if doc is None or vue is None:
            ...
            return
        # Un plan de surface EST un ViewPlan : le test de classe seul laisserait
        # passer un plan d'etage, ou aucune surface n'existe — le mode n'y
        # protegerait donc rien tout en affichant « actif ». Le ViewType tranche.
        if not isinstance(vue, _ViewPlan) or vue.ViewType != _ViewType.AreaPlan:
            self._echec(u"✖ La vue active doit être un plan de surface (Area Plan).")
            return
        ...
        #
        # Categories laissees modifiables : les lignes de separation de surface.
        # OST_AreaSchemeLines est celle que pose l'outil natif « Limite de
        # surface ». OST_AreaPolylines est exclue en plus, par precaution : elle
        # releve du meme trace de contour, et l'oublier reviendrait a epingler ce
        # que le mode est justement la pour rendre modifiable.
        #
        # getattr plutot qu'un acces direct : une categorie absente de
        # l'enumeration ne doit pas faire echouer toute la collecte.
        _exclues = [c for c in (getattr(_BIC, 'OST_AreaSchemeLines', None),
                                getattr(_BIC, 'OST_AreaPolylines', None))
                    if c is not None]
        if not _exclues:
            self._echec(u"✖ Catégorie des séparateurs de surface introuvable.")
            return

        # Option de la palette : les surfaces elles-memes peuvent rester
        # modifiables, pour ajuster un contour et sa surface dans la meme
        # passe. Les exclure du filtre revient a ne pas les epingler.
        if not self._verrouiller_surfaces:
            _surfaces = getattr(_BIC, 'OST_Areas', None)
            if _surfaces is not None:
                _exclues.append(_surfaces)
        ...
        ids = []
        tx = _Transaction(doc, u"NM-BATII — Activer l’édition des séparateurs de surfaces")
        tx.Start()
        try:
            for el in elements:
                try:
    ...
    def _restaurer(self, recuperation=False):
        ...
        if ids:
            tx = _Transaction(doc, u"NM-BATII — Désactiver l’édition des séparateurs de surfaces")
            tx.Start()
            try:
                for eid_int in ids:
                    try:
...
_CLE_ETAT = '__nm_batii_edition_separateurs_surfaces_etat__'
