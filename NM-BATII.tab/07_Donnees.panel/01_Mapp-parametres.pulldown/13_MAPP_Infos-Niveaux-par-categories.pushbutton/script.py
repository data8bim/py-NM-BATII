...
#__title__ = "Niveaux → Objets par niveau"
#__doc__ = """Transfert des valeurs de paramètres de niveaux vers les objets référencés à ce niveau.
#Description : Transfert des valeurs de paramètres des niveaux (ex: Nom) vers les objets (Pièces, Espaces, et tout élément dont le LevelId correspond) associés à chaque niveau.
#Permet de mapper des paramètres de niveaux vers des catégories d'objets choisies, afin de répercuter automatiquement les valeurs de chaque niveau sur les objets qui lui sont rattachés.
...
def _log(msg):
    if _LOGS_ENABLED:
        logger.debug(u'[Niveaux -> Objets] ' + msg)
...
#
_LAST_CFG = script.get_data_file('last_mapping', 'NM-Map-Niveaux')
...
def build_elements_by_level(doc, cat_names):
    """
    Pré-charge les éléments des catégories sélectionnées, regroupés par
    niveau (IntegerValue de LevelId). Un seul passage par catégorie,
    quel que soit le nombre de mappages qui la référencent.
    Exclut les Pièces/Espaces/Surfaces "Non placée(s)" (voir
    _is_unplaced_spatial_element) ainsi que tout élément sans LevelId
    valide.
    Retourne : {cat_name: {level_id_int: [elements]}}
    """
    result = {}
    for cat in cat_names:
        ...
        for elem in get_elements_for_category(doc, cat):
            ...
            if lid is None:
                continue
            by_level.setdefault(lid, []).append(elem)
        result[cat] = by_level
    return result
...
def apply_mappings(doc, rows):
    """
    Pour chaque niveau du projet, lit la valeur du paramètre source SUR
    CE NIVEAU, puis l'écrit sur les éléments des catégories sélectionnées
    dont le LevelId correspond à ce niveau.
    """
    levels = get_levels(doc)

    # ── 1. Pré-chargement des éléments par catégorie, groupés par niveau ─────
    unique_cats = set(cat for row in rows for cat in row.categories)
    elems_by_cat_level = build_elements_by_level(doc, unique_cats)
    ...
    try:
        with _pyrevit.Transaction(u'Niveaux -> Objets', doc=doc):
            for row in rows:
                for lvl in levels:
                    lid   = lvl.Id.IntegerValue
                    value = get_level_value(lvl, row.source_param)
                    prog.txtStatus.Text = u'{} : {} -> {}'.format(
                        lvl.Name, row.source_param, row.target_param)
                    _do_events()

                    for cat in row.categories:
                        for elem in elems_by_cat_level.get(cat, {}).get(lid, []):
                            try:
...
def _list_to_rows(data):
    # 'actif' absent : fichier enregistre avant l'ajout de la colonne « Actif »
    # — le mappage est actif, comportement identique a l'existant.
...
def load_config():
    ...
    if not dlg.FileName.lower().endswith('.nm-map-niveaux'):
        show_alert(
            u'Format incorrect',
            u'Fichier non valide.\n\nSeuls les fichiers ".NM-Map-Niveaux" sont acceptes.')
        return None
    try:
...
def make_row_border(row_data,
    ...
    cb_src, _ = _make_filterable_combo(
        ...
        on_src_select,
        Thickness(4, 0, 4, 0),
        u'Parametre source (niveau) — taper pour filtrer')
    ...
    btn_cat.ToolTip           = u'Selectionner les categories d\'objets cibles (references au niveau)'
...
def main():
    ...
    _log(u'{} params source (niveaux), {} categories'.format(
        len(source_params), len(all_categories)))

    if not source_params:
        show_alert(u'Niveaux -> Objets',
                   u'Aucun parametre trouve sur les niveaux du projet.')
        return
    ...
    table = MappingTableView(
        wpf, row_list,
        label_source=u'Paramètre source (Niveaux)',
    ...
    # Toujours au moins une ligne de mappage, meme vierge : sans elle le
    # tableau reste vide et se confond visuellement avec l'en-tete situe juste
    # au-dessus (confusion constatee a l'usage).
    if not row_list:
        add_row()
