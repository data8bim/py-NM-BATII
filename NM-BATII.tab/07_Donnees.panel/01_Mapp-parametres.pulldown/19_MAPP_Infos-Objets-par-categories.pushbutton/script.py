...
#__title__ = "Objets → Objets par catégories"
#__doc__ = """Recopie la valeur d'un paramètre source vers un paramètre cible du même objet.
#Description : Pour chaque objet des catégories sélectionnées, copie la valeur d'un paramètre source vers un autre paramètre du même type de données (texte, longueur, entier...) sur l'objet lui-même.
#Permet de mapper des paramètres entre eux sur un même élément, pour les catégories choisies dans chaque mappage.
...
def _log(msg):
    if _LOGS_ENABLED:
        logger.debug(u'[Objets -> Objets] ' + msg)
...
#
_LAST_CFG = script.get_data_file('last_mapping_objets', 'NM-Map-Objets')
...
def apply_mappings(doc, rows):
    """
    Pour chaque objet des catégories sélectionnées dans un mappage, lit la
    valeur du paramètre source SUR CET OBJET, puis l'écrit sur son propre
    paramètre cible (même élément).
    """
    # ── 1. Pré-chargement des éléments par catégorie ─────────────────────────
    unique_cats  = set(cat for row in rows for cat in row.categories)
    elems_by_cat = {cat: get_elements_for_category(doc, cat) for cat in unique_cats}

    total = 0
    for row in rows:
        for cat in row.categories:
            total += len(elems_by_cat.get(cat, []))

    # ── 2. Fenêtre de progression ─────────────────────────────────────────────
    xaml = script.get_bundle_file('ProgressWindow.xaml')
    prog = forms.WPFWindow(xaml)
    prog.Title               = u'Objets -> Objets'
    ...
    try:
        with _pyrevit.Transaction(u'Objets -> Objets', doc=doc):
            for row in rows:
                ...
                for cat in row.categories:
                    for elem in elems_by_cat.get(cat, []):
                        try:
...
def load_config():
    ...
    if not dlg.FileName.lower().endswith('.nm-map-objets'):
        show_alert(
            u'Format incorrect',
            u'Fichier non valide.\n\nSeuls les fichiers ".NM-Map-Objets" sont acceptes.')
        return None
    try:
...
def make_row_border(row_data,
    ...
    cb_src, _ = _make_filterable_combo(
        ...
        on_src_select,
        Thickness(4, 0, 4, 0),
        u'Parametre source (objet) — taper pour filtrer')
    ...
    cb_tgt, _reload_tgt = _make_filterable_combo(
        ...
        on_tgt_select,
        Thickness(4, 0, 4, 0),
        u'Parametre cible (meme objet) — taper pour filtrer')
...
def main():
    ...
    _log(u'{} params source (objets), {} categories'.format(
        len(source_params), len(all_categories)))

    if not source_params:
        show_alert(u'Objets -> Objets',
                   u'Aucun parametre trouve sur les objets du projet.')
        return

    xaml_path = script.get_bundle_file('WPFWindow.xaml')
    wpf       = forms.WPFWindow(xaml_path)
    wpf.Title = u'Objets -> Objets'
