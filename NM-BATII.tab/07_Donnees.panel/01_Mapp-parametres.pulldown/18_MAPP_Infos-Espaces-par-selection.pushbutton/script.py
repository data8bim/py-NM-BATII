...
#__title__ = "Espaces → Objets par sélection"
#__doc__ = """Transfert des valeurs de paramètres d'espaces par sélection.
#Description : Transfert des valeurs de paramètres des espaces MEP (ex: Nom, Numéro) vers les objets (familles) sélectionnés dans la vue active.
#Chaque objet sélectionné reçoit la valeur de L'ESPACE QUI LE CONTIENT (retrouvée via sa propriété Space, calculée par Revit à partir de la géométrie) : la valeur transférée dépend donc de l'espace de chaque objet, pas d'un espace unique.
#Permet de mapper des paramètres d'espaces vers des objets sélectionnés a la souris, afin de répercuter automatiquement la valeur de l'espace de chaque objet sur ses propres paramètres.
...
def _log(msg):
    if _LOGS_ENABLED:
        logger.debug(u'[Espaces Sel] ' + msg)
...
#  Suffixe _sel pour ne pas écraser le fichier du script «par espace».
#
_LAST_CFG = script.get_data_file('last_mapping_espaces_sel', 'NM-Map-Espaces-Sel')
...
def _get_element_space_id(elem, phase):
    ...
    Association "Espace :" — retourne l'IntegerValue de l'espace, ou None.
      - Si l'élément EST un espace, il se référence lui-même.
      - Sinon, Element.get_Space(phase)/.Space : équipements, appareils, etc.
    NB : contrairement aux pièces, les espaces n'ont pas d'équivalent
    FromRoom/ToRoom pour les portes/fenêtres — une seule association possible.
    """
    try:
...
def get_available_categories(doc):
    ...
    sur le projet" et "Niveaux" (aucun paramètre d'espace ne peut leur être
    mappé).
    """
...
def load_config():
    ...
    Seuls les fichiers .NM-Map-Espaces sont acceptés (même format que le
    script «Espaces → Objets par catégories»). Le champ «categories»
    ...
    if not dlg.FileName.lower().endswith('.nm-map-espaces'):
        show_alert(u'Format incorrect',
                   u'Seuls les fichiers ".NM-Map-Espaces" sont acceptés.')
        return None
    try:
...
def apply_to_selection(row_list, elements):
    ...
    SON espace (via sa propriété Space, ou lui-même s'il est déjà un
    espace) puis on lit la valeur du paramètre source sur CET espace. Deux
    objets situés dans des espaces différents recevront donc des valeurs
    différentes.
    """
    ...
    spaces_by_id = dict((s.Id.IntegerValue, s) for s in get_spaces(doc))
    phase = _get_last_phase(doc)

    n_set = n_skip = n_nospace = 0
    errors = []

    # Context manager pyRevit 6.4.0 : commit auto, rollback auto sur exception
    try:
        with _pyrevit.Transaction(u'Espaces -> Objets (selection)', doc=doc):
            for elem in elements:
                ...
                for row in active:
                    ...
                    space = spaces_by_id.get(sid) if sid is not None else None
                    if space is None:
                        n_nospace += 1
                        continue
    ...
    _log(u'{} renseigne(s), {} ignore(s), {} sans espace'.format(
        n_set, n_skip, n_nospace))
    ...
    if n_nospace:
        lines.append(u'{} objet(s) exclu(s) (aucun espace associe ou non place)'.format(n_nospace))
    if errors:
...
def make_row_border(row_data, source_params, source_param_types, params_by_type,
    ...
    cb_src, _ = _make_filterable_combo(
        ...
        Thickness(4, 0, 4, 0), u'Parametre source (espace) — taper pour filtrer')
...
def main():
    ...
    if not source_params:
        show_alert(u'Espaces',
                  u'Aucun paramètre trouvé sur les espaces du projet.')
        return
    ...
    wpf.Title = u'Espaces -> Objets (sélection)'
    ...
    table = MappingTableView(
        ...
        label_target=u'Paramètre cible (sur les objets sélectionnés)',
...
class _ActionHandler(IExternalEventHandler):
    ...
    def GetName(self):
        return u"NM-BATII — Infos espaces par selection"
