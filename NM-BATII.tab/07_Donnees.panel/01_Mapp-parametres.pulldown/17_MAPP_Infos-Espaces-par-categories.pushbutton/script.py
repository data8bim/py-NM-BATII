...
#__title__ = "Espaces → Objets par espace"
#__doc__ = """Transfert des valeurs de paramètres d'espaces vers les objets référencés à cet espace.
#Description : Transfert des valeurs de paramètres des espaces MEP (ex: Nom, Numéro) vers les objets (équipements, appareils...) référencés à chaque espace via leur propriété Space, associés à chaque espace.
#Permet de mapper des paramètres d'espaces vers des catégories d'objets choisies, afin de répercuter automatiquement les valeurs de chaque espace sur les objets qu'il contient.
...
from Autodesk.Revit.DB.Mechanical import Space as _SpaceClass
import System
...
def _log(msg):
    if _LOGS_ENABLED:
        logger.debug(u'[Espaces -> Objets] ' + msg)
...
#
_LAST_CFG = script.get_data_file('last_mapping_espaces', 'NM-Map-Espaces')
...
# ─── Lecture Revit : espaces et leurs paramètres ─────────────────────────────
...
def _get_space_name(space):
    """Space.Name n'est pas exposé directement en IronPython : passer par le
    paramètre. L'API Revit reutilise les BuiltInParameter de Room pour les
    Espaces MEP (ROOM_NAME, ROOM_NUMBER...)."""
    try:
...
def get_spaces(doc):
    spaces = list(FilteredElementCollector(doc)
                  .OfCategory(BuiltInCategory.OST_MEPSpaces)
                  .WhereElementIsNotElementType())
    return sorted(spaces, key=lambda s: (s.Number or u'', _get_space_name(s)))


def get_space_params(doc):
    """Union des noms de paramètres présents sur les espaces du projet."""
    names = set()
    for space in get_spaces(doc):
...
def get_space_value(space, param_name):
...
def get_available_categories(doc):
    ...
    "Informations sur le projet" et "Niveaux" (aucun paramètre d'espace ne
...
def _get_last_phase(doc):
    """Dernière phase du projet (utilisée par Element.get_Space)."""
    try:
        phases = doc.Phases
        if phases.Size == 0:
            return None
...
def _get_element_space_id(elem, phase):
    """
    Association "Espace :" — retourne l'IntegerValue de l'espace, ou None.
      - Si l'élément EST un espace, il se référence lui-même (permet de
        mapper un paramètre d'espace vers un autre paramètre du même
        espace quand la catégorie « Espaces » est choisie comme cible).
      - Sinon, Element.get_Space(phase)/.Space : équipements, appareils,
        etc.
    NB : contrairement aux pièces, les espaces n'ont pas d'équivalent
    FromRoom/ToRoom pour les portes/fenêtres (pas de relation "hébergé
    entre deux espaces" dans l'API Revit) — une seule association possible.
    """
    try:
        if isinstance(elem, _SpaceClass):
...
def build_elements_by_space(doc, cat_names):
    """
    Pré-charge les éléments des catégories sélectionnées, regroupés par
    espace (IntegerValue de l'Id de l'espace) — voir _get_element_space_id.
    Un seul passage par catégorie, quel que soit le nombre de mappages qui
    la référencent.
    Exclut les Pièces/Espaces/Surfaces "Non placée(s)" (voir
    _is_unplaced_spatial_element) ainsi que tout élément sans espace
    associé valide.
    Retourne : {cat_name: {space_id_int: [elements]}}
    """
    ...
    for cat in cat_names:
        ...
        for elem in get_elements_for_category(doc, cat):
            if _is_unplaced_spatial_element(elem):
                continue
            sid = _get_element_space_id(elem, phase)
            if sid is not None:
                by_space.setdefault(sid, []).append(elem)
        result[cat] = by_space
    return result
...
def apply_mappings(doc, rows):
    """
    Pour chaque espace du projet, lit la valeur du paramètre source SUR
    CET ESPACE, puis l'écrit sur les éléments des catégories
    sélectionnées dont la propriété Space correspond à cet espace.
    """
    spaces = get_spaces(doc)

    # ── 1. Pré-chargement des éléments par catégorie, groupés par espace ─────
    unique_cats = set(cat for row in rows for cat in row.categories)
    elems_by_cat_space = build_elements_by_space(doc, unique_cats)

    total = 0
    for row in rows:
        for space in spaces:
            if _is_unplaced_spatial_element(space):
                continue
            sid = space.Id.IntegerValue
            for cat in row.categories:
                total += len(elems_by_cat_space.get(cat, {}).get(sid, []))

    # ── 2. Fenêtre de progression ─────────────────────────────────────────────
    xaml = script.get_bundle_file('ProgressWindow.xaml')
    prog = forms.WPFWindow(xaml)
    prog.Title               = u'Espaces -> Objets'
    ...
    try:
        with _pyrevit.Transaction(u'Espaces -> Objets', doc=doc):
            for row in rows:
                for space in spaces:
                    if _is_unplaced_spatial_element(space):
                        continue
                    sid   = space.Id.IntegerValue
                    value = get_space_value(space, row.source_param)
                    prog.txtStatus.Text = u'{} : {} -> {}'.format(
                        _get_space_name(space), row.source_param, row.target_param)
                    _do_events()

                    for cat in row.categories:
                        for elem in elems_by_cat_space.get(cat, {}).get(sid, []):
                            try:
...
def save_config(rows):
    ...
    dlg.Filter     = u'Fichiers de mappage (*.NM-Map-Espaces)|*.NM-Map-Espaces'
    dlg.DefaultExt = 'NM-Map-Espaces'
    dlg.FileName   = 'espaces_mappages.NM-Map-Espaces'
...
def load_config():
    ...
    if not dlg.FileName.lower().endswith('.nm-map-espaces'):
        show_alert(
            u'Format incorrect',
            u'Fichier non valide.\n\nSeuls les fichiers ".NM-Map-Espaces" sont acceptes.')
        return None
    try:
...
def make_row_border(row_data,
    ...
    cb_src, _ = _make_filterable_combo(
        ...
        on_src_select,
        Thickness(4, 0, 4, 0),
        u'Parametre source (espace) — taper pour filtrer')
    ...
    btn_cat.ToolTip           = u'Selectionner les categories d\'objets cibles (references a l\'espace)'
    ...
    for child in [chk_actif, cb_src, arrow, cb_tgt, btn_cat, btn_del]:
...
def main():
    ...
    source_params      = get_space_params(doc)
    source_param_types = get_space_param_types(doc)
    all_categories      = get_available_categories(doc)
    params_by_type      = get_params_by_exact_type(doc)

    _log(u'{} params source (espaces), {} categories'.format(
        len(source_params), len(all_categories)))

    if not source_params:
        show_alert(u'Espaces -> Objets',
                   u'Aucun parametre trouve sur les espaces du projet.')
        return

    if not get_spaces(doc):
        show_alert(u'Espaces -> Objets',
                   u'Aucun espace trouve dans le projet.')
        return

    xaml_path = script.get_bundle_file('WPFWindow.xaml')
    wpf       = forms.WPFWindow(xaml_path)
    wpf.Title = u'Espaces -> Objets'
    ...
    table = MappingTableView(
        wpf, row_list,
        label_source=u'Paramètre source (Espaces)',
        label_target=u'Paramètre cible (sur les objets)',
