...
#__title__ = 'FILTRES\npar copie'
#__doc__ = """Transfert les filtres d'une nomenclature
#Description: Copie les filtres d'une nomenclature source vers les nomenclatures de destinations sélectionnées.
...
# -------------------------
# Analyse et application des filtres
# -------------------------
def analyze_filters(schedule, doc):
    """Analyser tous les filtres d'une nomenclature"""
    ...
    log('  {} filtre(s)'.format(filter_count))
    
    for i in range(filter_count):
        filter_obj = defn.GetFilter(i)
        
        # Recuperer le champ associe
        field_id = filter_obj.FieldId
        ...
        # Recuperer les valeurs selon le type de filtre
        try:
            if filter_obj.FilterType == ScheduleFilterType.Equal:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} = "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.NotEqual:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} != "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.Contains:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} contient "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.NotContains:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} ne contient pas "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.BeginsWith:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} commence par "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.NotBeginsWith:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} ne commence pas par "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.EndsWith:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} finit par "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.NotEndsWith:
                filter_info['string_value'] = filter_obj.GetStringValue()
                log('    Filtre {}: {} ne finit pas par "{}"'.format(i, param_name, filter_info['string_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.Greater:
                filter_info['double_value'] = filter_obj.GetDoubleValue()
                log('    Filtre {}: {} > {}'.format(i, param_name, filter_info['double_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.GreaterOrEqual:
                filter_info['double_value'] = filter_obj.GetDoubleValue()
                log('    Filtre {}: {} >= {}'.format(i, param_name, filter_info['double_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.Less:
                filter_info['double_value'] = filter_obj.GetDoubleValue()
                log('    Filtre {}: {} < {}'.format(i, param_name, filter_info['double_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.LessOrEqual:
                filter_info['double_value'] = filter_obj.GetDoubleValue()
                log('    Filtre {}: {} <= {}'.format(i, param_name, filter_info['double_value']))
            
            elif filter_obj.FilterType == ScheduleFilterType.HasParameter:
                filter_info['has_parameter'] = filter_obj.HasParameter
                log('    Filtre {}: {} a parametre = {}'.format(i, param_name, filter_info['has_parameter']))
            
            elif filter_obj.FilterType == ScheduleFilterType.HasNoParameter:
                log('    Filtre {}: {} n\'a pas de parametre'.format(i, param_name))
            
        except Exception as e:
            log('    Filtre {}: Erreur lecture valeur - {}'.format(i, str(e)))
        
        result.append(filter_info)
    
    return result

def apply_filters(schedule, source_filters, doc):
    """Appliquer les filtres a une nomenclature cible"""
    ...
    existing_filter_count = defn.GetFilterCount()
    if existing_filter_count > 0:
        log('  Suppression de {} filtre(s) existant(s)...'.format(existing_filter_count))
        # Supprimer de la fin vers le debut pour eviter les problemes d'index
        for i in range(existing_filter_count - 1, -1, -1):
            defn.RemoveFilter(i)
    
    # APPLIQUER LES FILTRES
    log('  Application des nouveaux filtres:')
    for filter_info in source_filters:
        param_id = filter_info['param_id']
        param_name = filter_info.get('param_name', 'Unknown')
        ...
        if param_id in param_to_field:
            ...
            try:
                ...
                new_filter = ScheduleFilter(target_field_id, filter_type)
                
                # Definir la valeur selon le type
                if 'string_value' in filter_info:
                    new_filter.SetValue(filter_info['string_value'])
                elif 'double_value' in filter_info:
                    new_filter.SetValue(filter_info['double_value'])
                elif 'has_parameter' in filter_info:
                    new_filter.SetValue(filter_info['has_parameter'])
                
                # Ajouter le filtre
                defn.AddFilter(new_filter)
                
                filters_added += 1
                log('    + Filtre ajoute: {}'.format(param_name))
            
            except Exception as e:
                log('    ! Erreur ajout filtre {}: {}'.format(param_name, str(e)))
                filters_skipped += 1
        else:
    ...
    log('  ---')
    log('  Resultat: {} filtres ajoutes, {} ignores'.format(filters_added, filters_skipped))
...
# -------------------------
def main():
    log('# Copier filtres de nomenclatures')
    log('---')
    ...
    # 4. Analyser source
    source_filters = analyze_filters(source_schedule, doc)
    
    if not source_filters:
        show_xaml_message('Aucun filtre trouvé dans la nomenclature source !', title='Erreur')
        return
    ...
    t = Transaction(doc, 'Copier filtres de nomenclatures')
    t.Start()
    ...
    for idx, target in enumerate(target_schedules):
        ...
        try:
            applied = apply_filters(target, source_filters, doc)
            if applied > 0:
                success += 1
            log('---')
        except Exception as e:
