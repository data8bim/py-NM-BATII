...
from System.Windows.Controls import (
    Grid, ComboBox, Button, Border, ColumnDefinition, TextBlock
)
from System.Windows import (
    GridLength, GridUnitType, Thickness,
    HorizontalAlignment, VerticalAlignment, WindowState
)
from System.Windows.Input import Key as WpfKey
...
class MappingRow(object):
    def __init__(self, source=u'', target=u''):
...
def load_config():
    ...
    Les champs «categories» éventuellement présents dans le fichier (issus
    du script «Infos Projet → Objets par catégories») sont simplement ignorés.
    """
...
def apply_to_selection(row_list, elements):
    ...
    """
    active = [r for r in row_list if r.source_param and r.target_param]
    if not active:
...
def make_row_border(row_data, source_params, source_param_types, params_by_type,
                    row_list, panel, used_targets, all_reload_entries,
    ...
    for cd in [star_col(), fixed_col(30), star_col(), fixed_col(32)]:
    ...
    for child in [cb_src, arrow, cb_tgt, btn_del]: g.Children.Add(child)
    border.Child = g
    return border
...
def main():
    ...
    def add_row(row_data=None):
        ...
        b = make_row_border(
            ...
            row_list, wpf.mappingsPanel, used_targets, all_reload_entries,
            refresh_all_target_combos, apply_view)
