...
try:
    ...
    pass   # styles non disponibles → boutons gardent l'apparence WPF par défaut
...
def show_result_window(msg):
    ...
    try:
        ...
        forms.alert(msg, title=u'Infos Projet -> Objets')
...
def apply_mappings(doc, rows):
    ...
    try:
        ...
        prog.Close()
        forms.alert(u'Erreur de transaction :\n' + str(ex), title=u'Erreur')
        return
...
def _show_save_dialog(filepath):
    ...
    try:
        ...
        forms.alert(u'Configuration enregistree :\n' + filepath,
                    title=u'Sauvegarde')
...
def load_config():
    ...
    if not dlg.FileName.lower().endswith('.nm-map-infos-proj'):
        forms.alert(
            ...
            u'Seuls les fichiers ".NM-Map-Infos-Proj" sont acceptes.',
            title=u'Format incorrect')
        return None
    try:
        ...
        forms.alert(u'Erreur de lecture :\n' + str(ex), title=u'Erreur')
        return None
...
def main():
    ...
    if not source_params:
        forms.alert(u'Aucun parametre trouve dans les Informations Projet.',
                    title=u'Infos Projet -> Objets')
        return
