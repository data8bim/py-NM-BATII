...
if not isinstance(view, ViewPlan):
    forms.alert("Ouvrez d’abord une vue en plan (étage ou plafond).")
    script.exit()

level = view.GenLevel
if not level:
    forms.alert("Impossible de récupérer le niveau associé à la vue.")
    script.exit()
...
def get_room_text_params():
    ...
    if not rooms:
        forms.alert("Aucune pièce trouvée.")
        script.exit()
...
class ParamDialog(WPFWindow):
    ...
    def on_ok(self, sender, args):
        ...
        if not self.selected_param:
            forms.alert("Veuillez choisir un paramètre.")
            return
        try:
