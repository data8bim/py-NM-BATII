...
"""
NM-BATII — Simplification des séparateurs de pièce (séparateurs) de la vue active.

Interface :
  - "Sélection des séparateurs"      → l'utilisateur sélectionne les séparateurs à traiter
  - "Toutes les séparateurs de la vue" → traite tous les séparateurs de la vue active
...
class SeparateursFilter(ISelectionFilter):
    _cat_id = int(BuiltInCategory.OST_RoomSeparationLines)

    def AllowElement(self, elem):
        try:
            return (elem.Category is not None and
                    elem.Category.Id.IntegerValue == SeparateursFilter._cat_id)
        except Exception:
            return False
...
class ResultWindow(forms.WPFWindow):
    ...
    def __init__(self, message, title=u"Simplifier les séparateurs de pièce"):
    ...
    @staticmethod
    def show(message, title=u"Simplifier les séparateurs de pièce", exit_after=False):
...
class FenetrePrincipale(forms.WPFWindow):
    ...
    def __init__(self):
        ...
        self.action = None
        # Fenetre NON MODALE : les references vers l'ExternalEvent sont portees
        # par l'instance, car les globals du module sont vides des que Execute()
        # a rendu la main (cf [[palette-non-modale-globals-ironpython]]).
        ...
        # Placement AVANT l'affichage : la fenetre est de taille fixe (Width et
        # Height du XAML), sa taille est donc connue des maintenant. Positionner
        # ici plutot que sur Loaded evite tout saut a l'ecran.
        try:
            from System.Windows import WindowStartupLocation as _WSL
            self.WindowStartupLocation = _WSL.Manual
        except Exception:
            pass
        self._centrer_sur_revit()

    def _centrer_sur_revit(self):
        """
        Centre la fenetre — horizontalement ET verticalement — sur la zone de
        travail de l'ecran hebergeant Revit.
        ...
        Sans division par le facteur d'echelle, la fenetre part hors ecran des
        ...
        try:
            largeur = self.Width  or self.ActualWidth  or 400
            hauteur = self.Height or self.ActualHeight or 360

            gauche = zone_gauche + ((zone_droite - zone_gauche) - largeur) / 2.0
    ...
    def _planifier(self, action):
        """Masque la fenetre (le focus revient a Revit pour PickObjects) puis
        planifie le traitement dans l'ExternalEvent : c'est ce contexte qui fait
        afficher a Revit le panneau ruban « Autoriser selection multiple »."""
        self.action = action
        self.Hide()
        fenetre = self

        def _do():
            # Execute() a restaure les globals du module juste avant : les
            # fonctions du pipeline sont donc de nouveau resolvables.
            try:
                _traiter(action)
            finally:
                # Reaffiche la fenetre pour enchainer un autre traitement,
                # meme si le traitement a echoue.
                try:
                    fenetre.Show()
                    fenetre.Activate()
                except Exception:
    ...
    def _on_selection(self, sender, args):
        self._planifier('selection')

    def _on_toutes(self, sender, args):
        self._planifier('toutes')
...
def run_pipeline(working_set, prog_win):
    ...
    tg = DB.TransactionGroup(doc, u"Simplification des séparateurs de pièce")
    tg.Start()
    try:
...
# ── ExternalEvent : le traitement s'execute sur le thread Revit ────────────────
#
...
# direct. C'est l'UI de selection retenue pour uniformiser l'extension.
#
# Difficulte : apres le retour de Execute(), IronPython/pyRevit vide les globals
# du module, ce qui casserait TOUT le pipeline ci-dessus (doc, view, DB, _log,
# step2/4/5, SeparateursFilter...). Plutot que de convertir ces ~700 lignes en
# methodes, on en sauvegarde une COPIE au demarrage et on la restaure en tete de
# Execute() : le code existant redevient utilisable tel quel.
...
class _ActionHandler(IExternalEventHandler):
    ...
    def Execute(self, uiapp):
        # Remet le module dans l'etat du demarrage : toutes les fonctions et
        # classes de ce fichier redeviennent resolvables.
        try:
    ...
    def GetName(self):
        return u"NM-BATII — Simplification des separateurs de piece"


def _traiter(action):
    """Un cycle complet : selection eventuelle, pipeline, resultat.
    S'execute sur le thread Revit via l'ExternalEvent."""
    working_set = None
    if action == 'selection':
    ...
    if working_set is not None:
        mode_txt = u"[{} séparateur(s) sélectionné(s)]".format(len(working_set))
    else:
...
# ── Lancement : fenetre principale NON MODALE ─────────────────────────────────
# Le handler est instancie APRES toutes les definitions de ce fichier : c'est a
# cet instant qu'est pris le snapshot des globals restaure par Execute().

_action_handler = _ActionHandler()
_ext_event      = ExternalEvent.Create(_action_handler)

_fenetre = FenetrePrincipale()
_fenetre.Show()
