...
CONFIG_KEY = 'suppression_masse_etiquettes_pieces'
...
# ─── Collecte ─────────────────────────────────────────────────────────────────
def get_used_room_tag_type_ids():
    """IntegerValue des types d'étiquette réellement utilisés par au moins une
    étiquette de pièce du projet."""
    ids = set()
    for tag in get_project_room_tags():
        try:
            tag_type = tag.RoomTagType
...
def get_room_tag_types():
    """Retourne les FamilySymbol de la catégorie 'Étiquette de pièces'
    réellement utilisés par au moins une étiquette du projet, triés."""
    used_ids = get_used_room_tag_type_ids()
    types = list(
...
def filter_tags_by_rooms(tags, room_ids):
    result = []
    for tag in tags:
        try:
            room = tag.Room
            if room is not None and room.Id.IntegerValue in room_ids:
                result.append(tag)
        except Exception:
            pass
    return result
...
def get_all_plan_views():
    return [
        v for v in DB.FilteredElementCollector(doc).OfClass(DB.ViewPlan).ToElements()
        if not v.IsTemplate
    ]
...
_BIP_ROOM_NUMBER = _safe_bip('ROOM_NUMBER')
_BIP_ROOM_NAME   = _safe_bip('ROOM_NAME')
_BIP_ROOM_LEVEL  = _safe_bip('ROOM_LEVEL_ID')
...
def _elem_family_and_type(elem):
    ...
    exploitable (ex. certaines pièces)."""
    try:
...
def _room_fixed_columns_spec():
    """Libellé (localisé) + clé de binding des colonnes fixes du tableau de
    pièces, dans l'ordre Numéro / Nom / Niveau / Famille / Type."""
    return [
        (_bip_label(_BIP_ROOM_NUMBER, u'Numéro'), u'Number'),
        (_bip_label(_BIP_ROOM_NAME, u'Nom'), u'Name'),
        (_bip_label(_BIP_ROOM_LEVEL, u'Niveau'), u'LevelName'),
        (_bip_label(_BIP_FAMILY, u'Famille'), u'Family'),
        (_bip_label(_BIP_TYPE, u'Type'), u'TypeName'),
    ]


def _room_fixed_column_labels():
    return set(label for label, key in _room_fixed_columns_spec())
...
# ─── Pièces sélectionnées : collecte, tableau de choix ─────────────────────────
def get_all_project_rooms():
    """Toutes les pièces placées (avec Location) du projet."""
    rooms = list(
        DB.FilteredElementCollector(doc)
...
def _room_sort_key(row):
    """Clé de tri par défaut du tableau de pièces : niveau puis numéro."""
    try:
        level = row.room.Level
        level_name = _elem_name(level) if level else u''
    except Exception:
        level_name = u''
    try:
        number = row.room.Number or u''
    except Exception:
        number = u''
    return (_normalize(level_name), _normalize(number))


class _RoomRow(object, INotifyPropertyChanged):
    """Ligne du tableau de choix des pièces (voir _ViewRow)."""

    def __init__(self, room):
        self.room = room
        self._selected = False
        self._PropertyChanged = None
        self.Number = _builtin_param_value_str(room, _BIP_ROOM_NUMBER)
        self.Name = _builtin_param_value_str(room, _BIP_ROOM_NAME)
        self.LevelName = _builtin_param_value_str(room, _BIP_ROOM_LEVEL)
        self.Family, self.TypeName = _elem_family_and_type(room)
...
def build_room_rows(rooms, ordered_names):
    rows = []
    for r in rooms:
        row = _RoomRow(r)
        for i, name in enumerate(ordered_names):
            setattr(row, 'P{}'.format(i), _param_value_str(r, name))
        rows.append(row)
    return rows


def show_rooms_picker_dialog(all_rooms, current_selected_ids):
    """Tableur de choix des pièces à traiter : mêmes options que
    show_views_picker_dialog (filtres/tri par colonne, sélection multiple
    Maj+Clic), colonnes Numéro/Nom/Niveau en tête, triées par défaut par
    niveau puis numéro."""
    xaml_path = os.path.join(os.path.dirname(__file__), 'RoomsPickerDialog.xaml')
    wpf = forms.WPFWindow(xaml_path)

    # "Numéro"/"Nom"/"Niveau"/"Famille"/"Type" sont déjà couvertes par les
    # colonnes fixes Number/Name/LevelName/Family/TypeName (résolues par
    # BuiltInParameter) : on les retire des colonnes dynamiques pour éviter
    # un doublon.
    raw_names = [n for n in get_dynamic_param_names(all_rooms) if n not in _room_fixed_column_labels()]
    ordered_names = order_column_names(raw_names, pinned_order=[])
    all_rows = build_room_rows(all_rooms, ordered_names)
    all_rows.sort(key=_room_sort_key)
    for r in all_rows:
        r.Selected = r.room.Id.IntegerValue in current_selected_ids
    ...
    def _update_count():
        n = sum(1 for r in all_rows if r.Selected)
        wpf.txtCount.Text = u"({} pièce(s) sélectionnée(s))".format(n)
    ...
    for label, key in _room_fixed_columns_spec():
        col = DataGridTextColumn()
        col.Header = _make_header(label, key)
        col.Binding = Binding(key)
        col.IsReadOnly = True
    ...
    if not dialog_result['ok']:
        return None
    return set(r.room.Id.IntegerValue for r in all_rows if r.Selected)
...
def main():
    ...
    if not tag_types:
        show_alert(
            ...
            u"Aucune étiquette de pièce n'est utilisée dans ce projet."
        )
        return
    ...
    selection_state_projet = {'rooms': []}
    selection_state_vues = {'rooms': []}
    views_state = {'selected_view_ids': set(), 'candidate_views': None}
    rooms_state = {'candidate_rooms': None}
    dialog_result = {'ok': False}
    ...
    def _mk_select_rooms_handler(state, txt_name, target_radio):
        def handler(s, e):
            ...
            try:
                ...
                state['rooms'] = [r for r in rooms if r is not None and _is_placed_room(r)]
            except Exception:
                pass
            wpf.WindowState = WindowState.Normal
            wpf.Activate()
            getattr(wpf, txt_name).Text = u"({} pièce(s) sélectionnée(s))".format(len(state['rooms']))
            target_radio.IsChecked = True
        return handler

    def _mk_select_rooms_table_handler(state, txt_name, target_radio):
        def handler(s, e):
            """Sélection des pièces via un tableau (comme 'Choisir les vues...'),
            plutôt qu'en désignant les pièces dans une vue Revit."""
            if rooms_state['candidate_rooms'] is None:
                rooms_state['candidate_rooms'] = get_all_project_rooms()
            candidates = rooms_state['candidate_rooms']
            if not candidates:
                show_alert(u"Supprimer les étiquettes",
                           u"Aucune pièce placée n'est disponible dans ce projet.")
                return
            current_ids = set(r.Id.IntegerValue for r in state['rooms'])
            result = show_rooms_picker_dialog(candidates, current_ids)
            wpf.Activate()
            if result is not None:
                state['rooms'] = [r for r in candidates if r.Id.IntegerValue in result]
                getattr(wpf, txt_name).Text = u"({} pièce(s) sélectionnée(s))".format(len(state['rooms']))
                target_radio.IsChecked = True
        return handler
        ...
        def handler(s, e):
            ...
            if result is not None:
                ...
                if not (wpf.rbVuesSelectionneesToutes.IsChecked or
                        wpf.rbVuesSelectionneesRooms.IsChecked):
                    target_radio.IsChecked = True
        return handler
    ...
    def _on_ok(s, e):
        if not any(cb.IsChecked for cb, t, label in checkbox_items):
            ...
            return
        if wpf.rbProjetSelection.IsChecked and not selection_state_projet['rooms']:
            show_alert(u"Supprimer les étiquettes",
        ...
        if wpf.rbVuesSelectionneesToutes.IsChecked and not views_state['selected_view_ids']:
            ...
            return
        if wpf.rbVuesSelectionneesRooms.IsChecked:
            if not views_state['selected_view_ids']:
                show_alert(u"Supprimer les étiquettes",
                           u"Aucune vue sélectionnée. Cliquez sur \"Choisir les vues...\".")
                return
            if not selection_state_vues['rooms']:
                show_alert(u"Supprimer les étiquettes",
                           u"Aucune pièce sélectionnée. Cliquez sur \"Sélectionner\".")
                return
    ...
    def _update_scope_buttons(s=None, e=None):
        wpf.btnSelectRoomsProjet.IsEnabled = bool(wpf.rbProjetSelection.IsChecked)
        wpf.btnSelectViews.IsEnabled = bool(wpf.rbVuesSelectionneesToutes.IsChecked)
        wpf.btnSelectViews2.IsEnabled = bool(wpf.rbVuesSelectionneesRooms.IsChecked)
        wpf.btnSelectRoomsVues.IsEnabled = bool(wpf.rbVuesSelectionneesRooms.IsChecked)
        wpf.btnSelectRooms.IsEnabled = bool(wpf.rbVueSelection.IsChecked)

    for rb_name in (u'rbProjetToutes', u'rbProjetSelection',
                    u'rbVuesSelectionneesToutes', u'rbVuesSelectionneesRooms',
                    u'rbVueToutes', u'rbVueSelection'):
        getattr(wpf, rb_name).Checked += _update_scope_buttons
    _update_scope_buttons()

    wpf.btnDeselectAll.Click += _on_deselect_all
    wpf.btnSelectRoomsProjet.Click += _mk_select_rooms_table_handler(
        selection_state_projet, 'txtSelectionCountProjet', wpf.rbProjetSelection)
    wpf.btnSelectRoomsVues.Click += _mk_select_rooms_table_handler(
        selection_state_vues, 'txtSelectionCountVues', wpf.rbVuesSelectionneesRooms)
    wpf.btnSelectRooms.Click += _mk_select_rooms_handler(
    ...
    wpf.btnSelectViews2.Click += _mk_select_views_handler(wpf.rbVuesSelectionneesRooms)
    wpf.btnOK.Click += _on_ok
    wpf.btnCancel.Click += lambda s, e: wpf.Close()
    ...
    elif wpf.rbVuesSelectionneesRooms.IsChecked:
        scope = 'vues_selection_rooms'
    ...
    if scope == 'projet_toutes':
        tags = filter_tags_by_types(get_project_room_tags(), selected_type_ids)
        if tags:
            view_tags_pairs.append((None, tags))
    elif scope == 'projet_selection':
        room_ids = set(r.Id.IntegerValue for r in selection_state_projet['rooms'])
        tags = filter_tags_by_types(
            filter_tags_by_rooms(get_project_room_tags(), room_ids), selected_type_ids
        )
        if tags:
            view_tags_pairs.append((None, tags))
        ...
        for v in candidates:
            if v.Id.IntegerValue not in selected_ids:
                continue
            tags = filter_tags_by_types(get_view_room_tags(v), selected_type_ids)
            if tags:
                view_tags_pairs.append((v, tags))
    elif scope == 'vues_selection_rooms':
        selected_ids = views_state['selected_view_ids']
        candidates = views_state['candidate_views'] or []
        room_ids = set(r.Id.IntegerValue for r in selection_state_vues['rooms'])
        for v in candidates:
            if v.Id.IntegerValue not in selected_ids:
                continue
            tags = filter_tags_by_types(
                filter_tags_by_rooms(get_view_room_tags(v), room_ids), selected_type_ids
            )
            if tags:
                view_tags_pairs.append((v, tags))
    elif scope == 'vue_toutes':
        tags = filter_tags_by_types(get_view_room_tags(view), selected_type_ids)
        if tags:
            view_tags_pairs.append((view, tags))
    else:
        room_ids = set(r.Id.IntegerValue for r in selection_state['rooms'])
        tags = filter_tags_by_types(
            filter_tags_by_rooms(get_view_room_tags(view), room_ids), selected_type_ids
        )
        if tags:
            view_tags_pairs.append((view, tags))
    ...
    n_ok = 0
    n_skip = 0
    with revit.Transaction(u"Supprimer les étiquettes de pièces en masse"):
...
class _ActionHandler(IExternalEventHandler):
    ...
    def GetName(self):
        return u"NM-BATII — Suppression des etiquettes de pieces"
