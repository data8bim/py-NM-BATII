...
def main():
    try:
        ...
        t.Start()
        existing_names = {
            v.Name for v in FilteredElementCollector(doc).OfClass(ViewPlan)
        } | {
            v.Name for v in FilteredElementCollector(doc).OfClass(ViewDrafting)
        }
        for name, key in candidates:
            if name not in selected_files or name in existing_names:
                continue
